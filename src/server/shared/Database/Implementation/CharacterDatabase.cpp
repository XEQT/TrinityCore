...
*/  // Just for testing
    PrepareStatement(CHAR_SEL_ENUM, "SELECT c.guid, c.name, c.race, c.class, c.gender, c.playerBytes, c.playerBytes2, c.level, c.zone, c.map, c.position_x, c.position_y, c.position_z, "
                     "gm.guildid, c.playerFlags, c.at_login, cp.entry, cp.modelid, cp.level, c.equipmentCache, cb.guid, c.slot, c.petentry, cp.id "
                     "FROM characters AS c LEFT JOIN character_pet AS cp ON c.guid = cp.owner AND cp.id = c.petentry LEFT JOIN guild_member AS gm ON c.guid = gm.guid "
                     "LEFT JOIN character_banned AS cb ON c.guid = cb.guid AND cb.active = 1 AND c.petentry AND cp.id WHERE c.account = ? AND c.deleteInfos_Name IS NULL", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER, "SELECT guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
                     ...
                     "health, power1, power2, power3, power4, power5, instance_id, speccount, activespec, exploredZones, equipmentCache, knownTitles, actionBars, grantableLevels, currentpetslot, petentry "
    ...
    PrepareStatement(CHAR_UPD_CHARACTER, "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,playerBytes=?,playerBytes2=?,playerFlags=?,"
                     ...
                     "equipmentCache=?,knownTitles=?,actionBars=?,grantableLevels=?,online=?,currentpetslot=?,petentry=? WHERE guid=?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PET_SLOTS_DETAIL, "SELECT owner, id, entry, level, name, slot FROM character_pet WHERE owner = ? AND slot >= ? AND slot <= ? ORDER BY slot", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_PET_BY_ENTRY_AND_ID, "SELECT id, entry, owner, modelid, level, exp, Reactstate, slot, name, renamed, curhealth, curmana, abdata, savetime, CreatedBySpell, PetType FROM character_pet WHERE owner = ? AND id = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHAR_PET_BY_ENTRY_ONLY, "SELECT id, entry, owner, slot, name, PetType FROM character_pet WHERE owner = ? AND id = ?", CONNECTION_BOTH);
    PrepareStatement(CHAR_SEL_CHAR_PET_BY_SLOT_ONLY, "SELECT id, entry, owner, slot, name, PetType FROM character_pet WHERE owner = ? AND slot = ?", CONNECTION_BOTH);
