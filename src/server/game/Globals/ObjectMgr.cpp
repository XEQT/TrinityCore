    ...
    do
    {
        ...
        switch (got.type)
        {
            ...
            case GAMEOBJECT_TYPE_TRANSPORT:                 // 11
            {
                ...
                {
                    sLog->outError(LOG_FILTER_TRANSPORTS,"Gameobject (Entry: %u GoType: %u) is transport but does not have entries in TransportAnimation.dbc! Gameobject is obsolete.",
                        &got.entry, &got.type);
                    break;
                }
                ...
                if (uint32 frame = got.transport.startFrame)
                {
                    ...
                    {
                        sLog->outError(LOG_FILTER_TRANSPORTS,"Gameobject (Entry: %u GoType: %u) has data0=%u but this frame is not in TransportAnimation.dbc! May cause client crashes.",
                ...
                if (uint32 frame = got.transport.nextFrame1)
                {
                    if (itr->second.find(frame) == itr->second.end())
                    {
                        sLog->outError(LOG_FILTER_TRANSPORTS,"Gameobject (Entry: %u GoType: %u) has data6=%u but this frame is not in TransportAnimation.dbc! May cause client crashes.",
                            &got.entry, &got.type, frame);
                    }
                }
                ...
                if (uint32 frame = got.transport.nextFrame2)
                {
                    if (itr->second.find(frame) == itr->second.end())
                    {
                        sLog->outError(LOG_FILTER_TRANSPORTS,"Gameobject (Entry: %u GoType: %u) has data8=%u but this frame is not in TransportAnimation.dbc! May cause client crashes.",
                            &got.entry, &got.type, frame);
                    }
                }
                ...
                if (uint32 frame = got.transport.nextFrame3)
                {
                    if (itr->second.find(frame) == itr->second.end())
                    {
                        sLog->outError(LOG_FILTER_TRANSPORTS,"Gameobject (Entry: %u GoType: %u) has data10=%u but this frame is not in TransportAnimation.dbc! May cause client crashes.",
                            &got.entry, &got.type, frame);
                    }
                }
                break;
